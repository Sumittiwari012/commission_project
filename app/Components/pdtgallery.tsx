...
const Pdtgallery = (props: ModelPanel) => {
  ...
  return (
    <div ref={sectionRef} className="relative flex items-center justify-center min-h-screen p-4 bg-[#111] overflow-hidden">
      ...
      {/* BACKGROUND ADJUSTMENT: Increased opacity and added brightness filter */}
      <div 
        className="absolute inset-0 bg-cover bg-center opacity-50 transition-transform duration-[30s] ease-linear"
        style={{ 
          backgroundImage: `url(${props.bgUrl || 'https://images.pexels.com/photos/17485738/pexels-photo-17485738.png'})`,
          transform: isVisible ? 'scale(1.2)' : 'scale(1)',
          filter: 'brightness(0.9)' // Adjust this number between 0.1 and 1.5 for more/less brightness
        }}
      />
      
      <div className="relative z-10 w-[95vmin] h-[95vmin] max-w-[500px] max-h-[500px] bg-white/5 backdrop-blur-xl border border-white/10 p-4 shadow-2xl">
        <div className="grid grid-cols-3 grid-rows-3 w-full h-full gap-4 relative">
          {images.map((url, index) => {
            const { top, left } = getPositionStyles(index);
            const titles = ["WOMENS", "KIDS", "STUDIO", "COLLECTION", "ACCESSORIES", "EDITORIAL", "ARCHIVE", "LOOKBOOK", "CONTACT"];
