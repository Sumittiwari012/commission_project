  ...
  const products = [
    {
      ...
      top: '35%', left: '20%',
      href: '/products/classic-silk-saree-1',   // ← set your path here
    },
    {
      id: 'item-2',
      title: 'Crimson Drape',
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-0 items-stretch overflow-hidden">
            ...
            <div className="flex items-start md:items-center justify-center text-center relative min-h-[250px] md:min-h-[500px]">
              {products.map((item) => (
                ...
                >
                  {/* Product Thumbnail — fluid width & height */}
                  <div
                    className="relative group overflow-hidden shadow-lg cursor-pointer flex-shrink-0"
                  ...
                  <div className="w-1/2 md:w-full text-left md:text-center pl-6 md:pl-0 md:mt-6">
                    <h3
                      className="font-serif-custom uppercase tracking-widest mb-1 md:mb-2 text-[#2d2d2d]"
                      style={{ fontSize: 'clamp(14px, 1.4vw, 26px)' }}
                    >
                      {item.title}
                    </h3>
                    <p
                      className="leading-relaxed text-gray-700 line-clamp-4 md:line-clamp-none"
                      style={{ fontSize: 'clamp(10px, 0.85vw, 16px)' }}
                    >
                      {item.description}
                    </p>

                    {/* Shop Look → navigates to item.href */}
                    <Link
                      href={item.href}
                      className="inline-block mt-3 md:mt-4 border-b border-black uppercase hover:text-pink-600 hover:border-pink-600 transition-colors"
                      style={{ fontSize: 'clamp(9px, 0.65vw, 13px)', letterSpacing: '0.3em' }}
                    >
