...
function Craft() {
  ...
  const loomImageRef = useRef<HTMLImageElement | null>(null);
  ...
  useEffect(() => {
    ...
    const loomImg = new Image();
    loomImg.src = '/loomh.png';
    loomImg.onload = () => { loomImageRef.current = loomImg; };
    ...
    const animate = () => {
      ...
      if (loomImageRef.current) {
        ctx.drawImage(loomImageRef.current, 0, 0, canvas.width, canvas.height);
      } else {
  ...
  {crafts.map(({ label, src }) => (
    <div key={label} className="relative overflow-hidden group cursor-pointer" style={{ aspectRatio: '1.8' }}>
      <img
        src={src}
        alt={label}
